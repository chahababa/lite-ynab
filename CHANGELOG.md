...
## 2026-07-04 - 快速記帳版面防位移調整

### Changed

- 快速記帳頁區塊重排：金額 → 支付方式 → 數字鍵盤固定在上半部；分類快速格移到數字鍵盤下方
- 「已儲存」通知與備註欄一併移到鍵盤下方
- 原因：分類格載入較慢、儲存通知出現在頂端，都會把畫面往下推，導致輸入中按錯鍵盤；重排後上半部輸入區完全不受下方載入快慢影響
- 新增版面順序測試，固定「鍵盤在分類格上方」的配置

### Verified

- `npm run typecheck`
- `npm run test`（29 個測試檔、140 個測試通過）

## 2026-07-04 - 預算分配引導模式（wizard）

### Added

- 新增 `/budget-allocation/wizard` 引導式分配流程：確認本月收入 → 一次一個大項逐組分配 → 總結頁
- 「待分配」金額全程釘在頂端即時更新，分超過收入會警告
- 每個小項顯示「上月分配・實花・結轉」參考，並提供「照上月」「照固定」「歸零」一鍵帶入
- 每按「下一組」即批次儲存該組，儲存失敗會停在原步驟提示
- 預算分配頁頂端新增「開始本月分配」入口卡；原頁保留為分類／固定預算／支付方式的管理中心

### Verified

- `npm run typecheck`
- `npm run test`（29 個測試檔、139 個測試通過）

## 2026-07-04 - 快速記帳成為首頁

### Changed

- 打開 App（`/`）直接進入快速記帳頁——手機使用情境幾乎都是為了記帳；主控臺移至 `/dashboard`，由底部導覽列「主控臺」進入
- 快速記帳頁現在也顯示底部導覽列（原本隱藏），確保落地後仍可前往其他頁面
- 設定頁與快速記帳關閉鈕等指向主控臺的連結，全部更新為 `/dashboard`

### Verified

- `npm run typecheck`
- `npm run test`（28 個測試檔、137 個測試通過）

## 2026-07-03 - UX 改進：底部導覽、快速記帳命中率、防呆

### Added

- 新增底部導覽列（BottomNav）：主控臺／預算／記一筆（置中主按鈕）／交易／報表，一下直達，取代原本左下角的浮動頁面選單
- 交易刪除後可一鍵復原：刪除前自動備份整筆資料，Toast 顯示「復原」按鈕（停留 6 秒）
- 主控臺「最近交易」可直接點擊，跳轉交易頁並自動開啟該筆的編輯視窗（`/transactions?edit=<id>`）
- 設定頁帳號卡新增「登出」按鈕；主控臺頂部新增設定入口

### Changed

- 快速記帳的常用分類格子改為「依最近 30 天使用頻率自動排序」，常用但未標記快速的分類也會自動浮上來；沒有使用紀錄時退回手動標記
- 快速記帳的備註欄預設收摺（實際使用中約 9 成交易不寫備註），點「＋ 加備註」展開
- 主控臺月份切換改用與其他頁一致的 MonthSwitcher 元件
- 「含上月結轉」顯示文案全站統一，預算分配頁的小項也會顯示結轉金額

### Removed

- 移除 PageQuickNav 浮動頁面選單（由底部導覽列取代；設定與登出入口移至主控臺頂部與設定頁）

### Verified

- `npm run typecheck`
- `npm run test`（28 個測試檔、136 個測試通過）
