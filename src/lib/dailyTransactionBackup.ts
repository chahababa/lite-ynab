...
export async function previewDailyTransactionBackupToGoogleSheets(
  ...
  const spreadsheetId = options.spreadsheetId ?? process.env.GOOGLE_TRANSACTION_BACKUP_SHEET_ID;
  ...
  const [existingCurrent, existingHistory] = await Promise.all([
    transport.getValues(spreadsheetId, `${CURRENT_SHEET}!A2:Q`),
    transport.getValues(spreadsheetId, `${HISTORY_SHEET}!A2:T`),
  ]);
...
function buildHistoryPlan(currentRows: CurrentRow[], existingCurrent: GoogleSheetsRow[], existingHistory: GoogleSheetsRow[], detectedAt: string, backupRunId: string) {
  const previousById = new Map(existingCurrent.filter((row) => row[0]).map((row) => [String(row[0]), row]));
  ...
  const existingEventIds = new Set(existingHistory.map((row) => String(row[0] ?? "")).filter(Boolean));
  const events = { BACKFILL: 0, INSERT: 0, UPDATE: 0, DELETE: 0, deduped: 0 };
  const historyRows: GoogleSheetsRow[] = [];
  for (const candidate of candidates) {
    const eventId = sha256(canonicalJson([candidate.type, candidate.row[0], candidate.row[16]]));
