# Lite YNAB 使用說明書

## 這是什麼

Lite YNAB 是一套輕量化的記帳與預算管理工具，適合用來快速記錄支出、規劃每月預算、追蹤預算使用情況，以及查看支出報表。

目前專案採用：

- Next.js
- TypeScript
- Tailwind CSS
- Supabase

## 主要功能

- 登入與驗證
- 主控臺查看本月收入、已分配預算、最近交易
- 快速記帳
- 預算分配
- 預算使用情況查看
- 交易清單編輯與刪除
- 報表匯出 CSV / Excel
- 設定分類與支付方式

## 頁面導覽

- `/login`
  - 登入頁
- `/`
  - 主控臺
- `/quick-entry`
  - 快速記帳頁
- `/budget-allocation`
  - 預算分配頁
- `/budget-usage`
  - 預算使用情況頁
- `/transactions`
  - 交易清單頁
- `/reports`
  - 報表頁
- `/settings`
  - 系統設定頁

## 日常使用流程

### 1. 登入

進入系統後先登入，未登入時大多數頁面會自動導回登入頁。

### 2. 設定本月收入

到主控臺或預算分配頁輸入本月收入。

建議先完成這一步，再做後續預算分配，這樣「尚可分配」金額會更準確。

### 3. 分配本月預算

到預算分配頁，依照分類輸入本月預算。

你可以：

- 手動輸入每個分類的預算
- 複製上月預算
- 批次套用固定預算
- 調整快速記帳分類
- 新增、重新命名或刪除分類
- 管理支付方式

### 4. 記錄支出

有兩種常用方式：

- 到主控臺下方快速新增一筆支出
- 到 `/quick-entry` 用手機友善介面快速記帳

記帳時需要填寫：

- 金額
- 日期
- 分類
- 支付方式
- 備註

### 5. 查看是否超支

到 `/budget-usage` 可以看：

- 今日支出
- 本月支出
- 各分類已花多少
- 剩餘多少預算
- 哪些分類已經超支

### 6. 維護交易資料

到 `/transactions` 可查看與編輯完整交易清單。

你可以：

- 修改金額
- 修改日期
- 修改分類
- 修改支付方式
- 補上備註
- 刪除交易

### 7. 查看與匯出報表

點選底部「報表」，使用上方月份左右箭頭查看上個月或更早的報表。

桌機瀏覽器寬度達 1,024px 時，報表會自動展開成寬版雙欄；手機維持單欄，沒有需要手動開啟的「桌機模式」。如果桌機上仍看到單欄，可將瀏覽器視窗放大；更新後請重新整理頁面。

到 `/reports` 可查看：

- 收入
- 已分配金額
- 已支出金額
- 尚可分配金額
- 大項支出占比環圈圖，附金額與百分比
- 所選月份與前月的支出差額、增減百分比
- 截至所選月份的近六個月支出趨勢
- 細項分類支出、預算與前月比較
- 支付方式支出

比較使用已記錄的支出。本月尚未結束時會提示；前月為零時不計算增減百分比。旅遊、月份天數與漏記，都可能影響結果；目前不會自動調整成每日平均或產生預算分配建議。無支出紀錄的月份會以 0 顯示。

也可以匯出：

- CSV：摘要、六個月趨勢、細項分類與支付方式統計
- Excel：所選月份的摘要

### 8. 每月自動輸出到 Google Sheets

這個功能的用途是：每個月把 Lite YNAB 裡的上個月花費，自動整理成 Google Sheets。之後你不用每次登入系統看報表，也可以直接在同一份試算表裡回顧每個月的花費。

它會寫進這份 Google Sheet：`Lite YNAB 月報匯出`。

目前會整理成四個分頁：

- `Monthly Summary`
  - 每個月一列總表，例如總支出、總預算、剩餘或超支、交易筆數。
- `Category Breakdown`
  - 依大項與小項分類列出花費，方便看錢主要花在哪裡。
- `Transactions`
  - 該月的每一筆交易明細。
- `Export Log`
  - 每次同步的紀錄，用來追蹤哪一次有成功寫入。

日常使用時，你通常不需要手動操作。正式排程設定好後，建議每月 1 號凌晨自動跑一次，把上個月資料寫進 Google Sheets。

如果需要手動補跑某個月份，可以請 Hermes 或工程端呼叫受保護的同步 endpoint：

```text
/api/cron/monthly-expense-report/sheets?monthId=YYYY-MM
```

例如要補 2026 年 5 月，就指定：

```text
monthId=2026-05
```

重跑同一個月份不會把同月資料越疊越多。系統會保留其他月份資料，只替換同一個 `monthId` 的舊 rows；但 `Export Log` 會新增一筆同步紀錄，方便追蹤。

正式寫入前可以先 dry-run：

```text
/api/cron/monthly-expense-report/sheets?monthId=YYYY-MM&dryRun=1&includeTables=1
```

dry-run 的意思是「只預覽、不真的寫入 Google Sheets」，適合在第一次設定或修正問題時使用。

要讓這個功能真的能寫入 Google Sheets，部署環境必須設定三個 Google service account 相關環境變數：

- `GOOGLE_SHEET_ID`
- `GOOGLE_SERVICE_ACCOUNT_EMAIL`
- `GOOGLE_SERVICE_ACCOUNT_PRIVATE_KEY`

注意：private key 是機密，只能放在 Zeabur / server 的環境變數裡，不可以貼到 Notion、GitHub、README、聊天紀錄或任何公開文件。

另外，Google service account 的 email 必須被加入那份 Google Sheet，而且至少要有 Editor 權限；否則程式會看得到設定，但沒有權限寫入試算表。

## 初次開發安裝

### 1. 安裝套件

```powershell
npm install
```

### 2. 設定環境變數

建立或確認 `.env.local` 內容正確：

```env
NEXT_PUBLIC_SUPABASE_URL=
NEXT_PUBLIC_SUPABASE_ANON_KEY=
```

注意：

- 不要提交 `.env.local`
- 非必要不要修改既有 Supabase 專案環境值

### 3. 啟動開發環境

```powershell
npm run dev
```

### 4. 型別檢查與測試

```powershell
npm run typecheck
npm run test
```

## 使用上的注意事項

- 所有使用者可見文字以繁體中文、台灣用語為主
- 金額欄位為純數字輸入，通常會置中顯示
- 分類與支付方式是預算與交易的核心資料，刪除前要先確認影響
- 如果同時在 Windows 本機開著 `npm run dev`，避免再跑 `npm run build`，以免 `.next` 快取互相干擾

## 常見問題

### 看不到資料或被導回登入頁

通常代表登入狀態失效，請重新登入。

### 記帳成功但畫面沒更新

先重新整理頁面，若仍異常再確認 Supabase 寫入是否成功。

### 找不到分類或支付方式

請到設定頁或預算分配頁確認資料是否已建立，或是否被刪除。

### 匯出檔案打開後顯示亂碼

CSV 建議用支援 UTF-8 的工具開啟；若有格式需求，可優先使用 Excel 匯出版本。

## 文件參考

- `DEPLOYMENT.md`
  - 部署相關說明
- `CHANGELOG.md`
  - 變更紀錄
- `AGENTS.md`
  - 專案協作與操作規則

如果你想把這份文件改成更偏「給最終使用者」或更偏「給工程接手者」的版本，可以再往下拆成兩份。
